...
const CartItem = ({ item }) => {
  ...
  const idPayload = item.type === "bundle" ? { bundleId: item.bundleId } : { productId: item.productId, size: item.size };
    ...
    <div className="flex animate-fadeUp gap-4 border-b border-navy-100 py-5 last:border-0">
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            <div className="flex items-center gap-2">
              ...
              {item.type === "bundle" && (
                <span className="stamp bg-amber-50 text-amber-700 ring-1 ring-inset ring-amber-200">Bundle</span>
              )}
            </div>
        ...
        <div className="mt-3 flex items-center gap-2">
          <button
            onClick={() => updateQuantity({ ...idPayload, quantity: item.quantity - 1 })}

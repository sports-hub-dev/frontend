...
// APS (it requires a signed set of fields), a plain redirect for Stripe
// (it just needs the URL Stripe's Checkout Session already gives us).
...
const Checkout = () => {
  ...
  const handlePlaceOrder = async (address) => {
    ...
    setPaymentError("");
    try {
      // Neither branch clears the cart here — the order exists server-side now
      // (paymentStatus: "pending"), but if payment fails/is abandoned on the
      // gateway's page, the customer returns to /checkout?payment=failed and
      // needs their cart intact to retry. The cart only clears once payment is
      // confirmed — see OrderDetail.jsx, which clears it on ?payment=success.
      if (paymentMethod === "stripe") {
        const { data } = await paymentsApi.createStripeOrder(buildOrderPayload(address));
        window.location.href = data.data.stripe.checkoutUrl;
      } else {
  ...
  /**
   * TESTING ONLY — places the order via POST /orders directly, skipping both
   * payment gateways. Only rendered outside production builds.
   */
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
              {PAYMENT_METHODS.map((m) => (
                <button
                  ...
                  className={cn(
                    "btn-transition rounded-xl border p-4 text-left",
